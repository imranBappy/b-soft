            ...
            <div>
                <Link href='#' className=" block font-medium">{name}</Link>
                {
                    discount ? <div className='flex gap-2 items-center'>
                        ...
                        <span>|</span>
                        <p className="text-sm  text-gray-200">{quantity} x ${(price - discount).toFixed(2)}</p>
                    </div> : <p className="text-sm text-gray-200">{quantity} x ${(price - discount).toFixed(2)}</p>
                }

            </div>
            <div className='flex'>
                <div className='flex flex-col gap-[2px]'>
                    <Button onClick={onMinusItem} variant="secondary" className='hover:bg-red-500  rounded-none  h-5 hover:text-white' size="sm">
                        <Minus />
                    </Button>
