...
export function ProductForm({ id }: { id?: string }) {
    ...
    const [createProduct, { loading: create_loading }] = useMutation(
        PRODUCT_MUTATION,
        {
            refetchQueries: [
                {
    ...
    const { loading: product_loading, data: product_res } = useQuery(
        PRODUCT_QUERY,
        {
            ...
            onCompleted: async (data) => {
                const photo = await jsonToImages(data?.product?.photo).map(
                    (item: string) => ({
                        id: randomId(),
                        file: null,
                        url: item,
    ...
    const { data: categories_res, loading: categories_loading } =
        useQuery(CATEGORIES_QUERY);

    const categories: OPTION_TYPE[] = categories_res?.categories.edges.map(
    ...
    async function onSubmit(data: ProductFormValues) {
        try {
            ...
            for (let i = 0; i < preImages.length; i++) {
                const findImage = uploadedFiles.find(
                    (item) => item === preImages[i]
                );
                if (!findImage) deletedImage.push(preImages[i]);
            }

            if (deletedImage.length) {
                const deletedPromises = deletedImage.map(
