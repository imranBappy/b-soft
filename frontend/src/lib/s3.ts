...
export async function uploadImageToS3(file: File) {
    const s3Client = new S3Client({
        ...
        credentials: {
            ...
            secretAccessKey: process.env
                .NEXT_PUBLIC_BUCKET_SECRET_ACCESS_KEY as string,
        },
    });
    ...
    try {
        ...
        return `https://s3.ap-south-1.amazonaws.com/b-soft-s3.xyz/${params.Key}`;
    } catch (error) {
        ...
        const s3Client = new S3Client({
            ...
            credentials: {
                accessKeyId: process.env
                    .NEXT_PUBLIC_BUCKET_ACCESS_KEY as string,
                secretAccessKey: process.env
                    .NEXT_PUBLIC_BUCKET_SECRET_ACCESS_KEY as string,
            },
        });
